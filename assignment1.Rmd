...
### 8. Test the normality of homeprice. Would this test result change your responses to #1) or to 7)? Why or why not
```{r}
...
```
* By using the histogram, qq-plot, and SW normality test (p<0.005), we can reject the null that the distribution of homeprice is normal. 
* This does not change our assessment to #1, as I did not state that homeprice was normally distributed with the said mean and said variance. It was actually my suspicison since I observed a positive right skew in the data. 
* Nor changes our assessment to #7 because non-normality is not relevant in stating correlation does not equal causation. Causation can only be determine by controlling variables in an experiment, or using particularly strong quasi-experimental methods. 
